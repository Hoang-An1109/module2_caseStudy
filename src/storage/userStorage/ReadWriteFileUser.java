    ...
    public void writeFile(List<User> userList) {
        try {
            ...
            for(User user: userList){
                userString.append(user.getId()).append(",").append(user.getUserName()).append(",").append(user.getPassword()).append(",").append(user.getFullName()).append(",").append(user.getGender()).append(",").append(user.getDateOfBirth()).append(",").append(user.getRole()).append(",").append(user.getPhoneNumber()).append("\n");
            }
    ...
    public List<User> readFile() {
        ...
        try {
            ...
            while ((line= bufferedReader.readLine())!=null){
                ...
                LocalDate date= LocalDate.parse(userString[5].trim());
                String role=userString[6].trim();
                String phone=userString[7].trim();

                User user=new User(id,username,password,fullName,gender,date,role,phone);
                userList.add(user);
            }
